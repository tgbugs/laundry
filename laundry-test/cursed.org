...
# ox-html produces (underline "{" (underline "{_"))
# which I guess is technically correct here?
...
check this font locking bug out! want to see a newline render with underline IN A COMMENT !?
# {_x
_{_{___
...
* script in footnote cursedness
it seems that there are certain forms that don't "actually" count as
"not whitespace" because the paragraph parsing starts as if from
nothing (aka a newline) inside of them

inline [fn::^{is this superscript?}] no, it is not
inline [fn::_{is this subscript?}] no, it is not

[fn::a] wait, since when can we start a line with an inline footnote!?
...
ok, so the spec is inaccurate, a footnote label cannot be a colon

[fn:29:THERE's MORE] oh boy

now reference [fn:29] here as well

#+begin_src elisp
(string-match "[[:word:]]" "Ǿ") ; does not match racket's [[:word:]]
#+end_src

* footnote hyperlink italic interactions
[s/] [[/ ][]]
...
oh body [[https://www.crossref.org/services/event-data/ ][Cross Ref’s Event]] hrm
...
[_@@html: html@@@@latex: latex@@_]

* timestamps

org export shows that the first 3 are timestamps
and the last one is a hyperlink
